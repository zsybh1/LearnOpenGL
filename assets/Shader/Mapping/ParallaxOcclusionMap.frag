...
vec2 ParallaxMapping(vec3 viewDir, vec2 texcoord) {
    ...
    float prevMapDepth = texture(depthMap, prevTexcoord).r;
    float prevDepthDiff = prevMapDepth - curDepth + layerDepth;
    float curDepthDiff = curMapDepth - curDepth;

    float ratio = curDepthDiff / (curDepthDiff - prevDepthDiff);
    vec2 finalTexcoord = curTexcoord * (1.0 - ratio) + prevTexcoord * ratio;
